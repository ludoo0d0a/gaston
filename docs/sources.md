...
**API keys:** which feeds need credentials and how to register → [`API_KEYS.md`](API_KEYS.md). Build-time property names → [`ENV_VARS.md`](ENV_VARS.md). EU EV stations + live availability NAPs → [`EV_AVAILABILITY_ROADMAP.md`](EV_AVAILABILITY_ROADMAP.md). Non-EU (US, CA, AU, JP, CN) → [`EV_AVAILABILITY_ROADMAP_NON_EU.md`](EV_AVAILABILITY_ROADMAP_NON_EU.md).
